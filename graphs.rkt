...
;; o veți implementa obligatoriu folosind funcționale și/sau funcții anonime, fără recursivitate
  ...
  (define (incoming G v)
    (foldl (λ (x res)  ; parametrii sunt pe invers fata de outgoing
             (if (equal? v (cadr x))
                 (append res (list (car x)))
                 res))
           '() (get-edges G)))
...
;; Folosiți let-uri și/sau funcționale.
  ...
  ;(dfs-rec G (list (cons v  'none)) null null))
  ;(define (dfs G v)
    
    (let dfs-rec ((stack (map (λ (x) (cons x 'none)) (list v))) ;(get-nodes G)))
                 (visited null))
      (if (null? stack)
          visited
          (let ((nod (caar stack))
                (rest (cdr stack)))
            (if (member nod (map car visited))
                        (dfs-rec rest visited)
                        (dfs-rec (append (map (λ (x) (cons x nod)) (outgoing G nod)) rest)
...
(define (dfs G)
     (foldl (λ (v acc) (let dfs-rec ((stack (map (λ (x) (cons x 'none)) (list v)))
                                      (visited acc))
     (if (null? stack)
          visited
          (let ((nod (caar stack))
